...
- **A new site ships a whole content-script bundle, not a diff.** Each `entrypoints/<site>.content.ts` compiles to its own standalone file carrying the full shared UI (`src/ui/`, `src/shared/`, preact) plus that one adapter. The copies cannot be folded into a shared chunk: WXT builds every content script in Vite library mode as a single IIFE, and Rollup does not code-split IIFE output — the same constraint [localization.md](localization.md) works around by fetching emoji data as a static file instead of importing it. WXT lists content-script chunking as planned, not shipped, so this is upstream to revisit rather than something to work around here.

  Two consequences worth knowing before you size an adapter. The `pnpm check:bundle` budget is **per bundle**, and most of each bundle is that shared floor — the gate prints every bundle's current size when it runs, so read the real numbers there rather than guessing which side of the budget your adapter lands on. And growth in `src/ui/` is paid by every site at once, which is why a per-site helper belongs in the adapter rather than in the shared layer. What this does *not* cost is page weight: a page loads exactly 1 content script, so the duplication is package size and store-review surface, never per-page work.
...
Create `src/entrypoints/<site>.content.ts` with `matches: matchPatternsForSite("<site>")` from `src/shared/sites.ts`: WXT evaluates the entrypoint's options at build time, so the manifest's `content_scripts` entry comes straight from the registry row. `src/shared/content-matches.test.ts` checks the file names its own site for both the hosts and the adapter it wires, so a copy-pasted entrypoint that still points at another site fails `pnpm test`.
