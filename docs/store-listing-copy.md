...
No account is needed to read counts. To add a reaction, sign in with an account you already have (Google, Apple, Microsoft or Twitch). These reactions are counted separately from the site's native Likes, stars and votes.
...
```text
Used to save extension settings, per-site preferences, the signed-in session (the session token and the name of the provider it was opened with, kept only to label the account in the popup), the name and emoji mark this device shows for each account signed in on it, the settings each of those accounts changed away from the defaults, a random installation identifier, recently used emoji, local reaction state and a 60-second cache of counts. Only the settings object syncs through the browser's extension-settings sync; the session, the per-account records, the identifier, the cache and the reaction state stay on the device. These values support the reaction picker, the account session and consistent behavior between visits.
```
...
The reaction buttons and counts appear on the supported sites (x.com, facebook.com, reddit.com, instagram.com, youtube.com, github.com, gitlab.com, threads.com, amazon.com, rottentomatoes.com, themoviedb.org, imdb.com, metacritic.com, opencritic.com) without signing in — install and browse any of them. github.com/torvalds/linux is a reliable page to check without signing in.
...
```text
No account is needed for the core features. Install the extension and open any supported page (youtube.com, reddit.com, github.com, gitlab.com, x.com, threads.com, facebook.com, instagram.com, amazon.com, rottentomatoes.com, themoviedb.org, imdb.com, metacritic.com, opencritic.com): the reaction button appears next to the site's own controls and shows the public counts. github.com/torvalds/linux is a reliable page to check without signing in.
...
No account is needed to see the reaction button and the counts. Install the extension and open any supported page (youtube.com, reddit.com, github.com, gitlab.com, x.com, threads.com, facebook.com, instagram.com, amazon.com, rottentomatoes.com, themoviedb.org, imdb.com, metacritic.com, opencritic.com). github.com/torvalds/linux is a reliable page to check.
