...
No account is needed to read counts. To add a reaction, sign in with an account you already have (Google, Apple, Microsoft, Twitch or Slack). These reactions are counted separately from the site's native Likes, stars and votes.
...
To test voting (optional): open the extension, choose Sign in, tick the consent box and pick a provider (Google, Apple, Microsoft, Twitch or Slack). The provider's own login page opens in a browser window; sign in there with any account you have and the extension's page confirms the sign-in. No account details reach the add-on: the provider's answer is exchanged for a session by the backend.
...
To test reacting: open the extension popup, choose Sign in, tick the consent box and pick a provider (Google, Apple, Microsoft, Twitch or Slack); sign in on the provider's own page in the window that opens.
...
To test reacting: open the popup, choose Sign in, tick the consent box and pick a provider (Google, Apple, Microsoft, Twitch or Slack); sign in on the provider's own page in the window that opens. The identity permission is what opens that window; the extension never hosts the provider's form.
