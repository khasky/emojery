...
- `pnpm run build:chrome` then `pnpm run test:e2e` builds the staging extension (`.output/chrome-mv3-staging`) and runs the deterministic Playwright e2e suite against the public staging backend.
- Optional Firefox pass: `pnpm run build:firefox` then `E2E_BROWSER=firefox pnpm run test:e2e` — the same suite in Playwright Firefox. Specs that need the extension's own pages (popup/auth) skip there; see [e2e/README.md](../e2e/README.md).
...
**Only a production build reaches the production API.** The `production` mode is what compiles in `https://api.emojery.app`: `pnpm build:production` and `pnpm zip:production`, plus their `:chrome` / `:firefox` / `:safari` variants. Every other build, `pnpm build` and `pnpm zip` included, targets staging and lands in the `-staging` output directory and archive name. `scripts/scan-extension-artifact.sh` refuses an artifact carrying the staging origin, so a build made with the wrong script cannot be packed into a store upload by mistake. Both workflows run the production scripts, and so does `pnpm release`; a hand-run upload uses them too.
...
- **`main` is the permanent branch and the next line.** Every change lands there first, experiments included, and every minor is tagged on it. A `pnpm build` of `main` is where the next minor is exercised before it ships.
...
pnpm first runs the `prerelease` lifecycle hook (`pnpm check` — the same gate listed above) automatically, so a failing check aborts the release before anything is tagged. The script then derives the next SemVer version from Conventional Commits since the last `v*` tag, updates `package.json` and `CHANGELOG.md`, creates the release commit and tag, and runs `zip:production`:
...
Safari is built only here, by `pnpm zip:production:safari` — `zip:production` (what `pnpm check` and every PR run) stays on Chromium and Firefox, so a Safari-only build break surfaces at the tag rather than on the PR.
...
`pnpm release` is not used here: pnpm appends extra arguments to the end of the script, which for `release` is `zip:production` rather than `commit-and-tag-version`, so `--release-as` would reach the wrong command. Running `pnpm check` first restores what the `prerelease` hook would have done, and the workflow builds the zips from the tag anyway.
