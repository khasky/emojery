...
// local identifier (the queued vote's optimistic history row, the install id
// sent as a request header). Not a secret and not a hash of anything
// user-derived - just randomness.
//
// The output always matches /^[A-Za-z0-9_-]{16,128}$/ so a stored id survives
// the readback validation in background/api-client.ts.
...
export function randomId(): string {
  ...
  // predictable id would be worse than none, and background/api-client.ts lets
  // the throw fail the request rather than send it without an install id.
  // Unreachable in a browser; failing loud is what keeps it that way.
