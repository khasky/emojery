...
What it covers: every content-script surface — placement, picker DOM, theming, replace-native detection, private-page gate, message boundary — and, through **`lib/firefox-bridge.ts`**, the extension's own contexts: sign-in and sign-out (the OTP runtime messages, sent from a popup tab), the Settings/Account toggles (the popup driven by an evaluated function), history reads (`history:page`), background-context seeding (`evalInBackground`, the twin of `firstServiceWorker().evaluate`), the auth tab a gate click opens, and the extension pages as sized windows for `a11y-firefox.spec.ts`. The bridge is the same remote debugging protocol socket the temporary-add-on install uses: Playwright's juggler neither navigates to nor tracks `moz-extension://` pages (verified: even a tab the background opens via `tabs.create` never appears in `context.pages()`), but RDP reaches the background page and every extension tab, so anything expressible as "run this function in the page and judge its return value" runs there. What still skips (with the reason in the report): specs that need Playwright's own page machinery on those pages — locators, aria snapshots, screenshots and visual baselines, trusted keyboard input, downloads (the popup-locator cases of `auth`, `history-transfer`, `visual`, `settings-extras`, `persistence`, `authed-extras`, `a11y`); `locale-sweep` (Playwright's Firefox ships no language packs, so `browser.i18n` stays en-US whatever `intl.locale.requested` says — verified); the onboarding tab (a temporary add-on install skips it by design); and the chromium-only mechanics: CDP perf budgets, coexistence (Chrome MV3 sources), the private-window case. Firefox function bodies handed to the bridge must be self-contained and reach the API as `(globalThis.browser ?? chrome)` — Firefox's `chrome.*` returns no promises.
...
| --- | --- | --- | --- |
| `site-injection.spec.ts` | Per-site placement, replace-native, localized placement, and the gated authed loop. Its scenario table is data-only in **`supported-sites.ts`**. | Only the authed loop | Every other day (`e2e-ci.yml`), the placement subset again in Firefox every scheduled slot (same workflow, firefox job); the replace-native loop weekly; the GitHub + YouTube placement pair again in real Edge every other day (`edge-smoke.yml`); and the placement scenario alone on any PR touching `src/adapters/` (`e2e-adapter.yml`, advisory) |
...
| `settings-extras.spec.ts` | The "Reaction animations" toggle, a second open popup not live-syncing a settings change (characterized), the popup reopening on the tab it was left on, per-site off restoring the native control replace-native hid, the Theme setting repainting a live trigger and the extension's own pages, and Debug revealing the queue panel | Only the animations cases | The Theme + Debug cases every scheduled slot (`e2e-ci.yml`, own-surfaces job); the rest manual |
| `history-transfer.spec.ts` | Export writes a real download the browser saves, and importing a modified copy of it REPLACES the stored rows (a cancelled confirm and a foreign JSON change nothing) | Yes | Manual only |
| `visual.spec.ts` | Pixel baselines for the extension's own pages — popup (Settings, per-site, Account, Report, Debug), auth, onboarding — in light and dark plus the German length case. Compared **only under CI** (`ignoreSnapshots`) | No | Every scheduled slot (`e2e-ci.yml`, own-surfaces job) |
| `locale-sweep.spec.ts` | All 26 shipped locales on popup/auth/onboarding: the translation is substituted, no message key leaks as raw text, and nothing overruns the popup's real 360px width | No | Every scheduled slot (`e2e-ci.yml`, own-surfaces job) |
...
## Own-surface baselines (visual + locales)

Everything above measures the extension against somebody else's page. These two run against the pages Emojery draws itself, so they need no site, no account and no network - the whole job is ~3 minutes.

`visual.spec.ts` holds the pixel baselines: popup (Settings, the per-site list, Account and Report signed out, the Debug queue), `auth.html` and `onboarding.html`, each in light and dark, plus a German pass where the labels are longest. The popup is shot at 360px - `min-width` in popup.css, the width Chrome gives the real popup - so a translation that overruns it fails here rather than in a bug report. The build stamp is masked; it changes every release and nothing else about the header does.

**Baselines are per-platform, and only CI compares them.** `ignoreSnapshots` in `playwright.config.ts` switches the comparison off everywhere except CI, because a contributor on Windows or macOS would otherwise diff their own text rendering against the Linux baselines. To refresh: dispatch **E2E (CI)** with `update_snapshots`, download the `visual-baselines` artifact, commit it. A local run writes `*-win32.png` / `*-darwin.png` files that nothing reads - `.gitignore` keeps them out of the tree.

`locale-sweep.spec.ts` walks **all 26 shipped locales** (`E2E_LOCALES=de,ru` narrows it) and, per locale, launches Chrome with that UI language and asserts on popup, auth and onboarding: the tabs and one Settings row carry the catalog's own translation (falling back to English exactly where Chrome would), no message key leaks through as raw text, and at 360px nothing scrolls sideways or hangs past the popup's edge. `i18n-locales.test.ts` checks the placeholder definitions and the translation backlog - it never loads a page, which is why a string that fits the catalog but not the layout only shows up here. `a11y.spec.ts` owns the reflow breakpoint for auth/onboarding in one language; this is the same question asked in all of them.
