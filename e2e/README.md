...
`locale-sweep.spec.ts` walks **all 26 shipped locales** (`E2E_LOCALES=de,ru` narrows it) and, per locale, launches Chrome with that UI language and asserts on popup, auth and onboarding: the tabs and one Settings row carry the catalog's own translation (falling back to English exactly where Chrome would), no message key leaks through as raw text, and at 360px nothing scrolls sideways or hangs past the popup's edge. `verify:locales` checks the placeholder definitions and `i18n-locales.test.ts` the translation backlog - neither one loads a page, which is why a string that fits the catalog but not the layout only shows up here. `a11y.spec.ts` owns the reflow breakpoint for auth/onboarding in one language; this is the same question asked in all of them.
