...
import { authAccount, authConfigured, completeSignIn, envUrl, signInSkipReason } from "./lib/test-config";
...
test("the gate's own auth tab returns to the page and closes itself", async () => {
  ...
  try {
    ...
    await signInOnAuthPage(authPage, { completeSignIn: (window) => completeSignIn(window, account) });
    signedIn = true;
